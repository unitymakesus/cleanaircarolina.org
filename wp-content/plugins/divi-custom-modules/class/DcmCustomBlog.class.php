 ...
 * Time: 00:08
 */
class GWP_Custom_Module_Blog extends ET_Builder_Module{
	function init() {
        $this->name       = esc_html__( 'Custom - Blog', 'et_builder' );
        $this->slug       = 'et_pb_post_custom';
    ...
    static function get_blog_posts( $args = array(), $conditional_tags = array(), $current_page = array() ) {
        ...
        if ( $query->have_posts() ) {
            ...
            while( $query->have_posts() ) {
                ...
                $thumb          = '';
                $width          = 'on' === $args['fullwidth'] ? 350 : 400;
                $width          = (int) apply_filters( 'et_pb_blog_image_width', $width );
                $height         = 'on' === $args['fullwidth'] ? 210 : 250;
                ...
                // Print output
                ?>
                <article id="" <?php post_class( 'et_pb_post dcm_post' . $no_thumb_class . $overlay_class ) ?>>
                    <?php
                    ...
                    <?php if ( 'off' === $args['fullwidth'] || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) ) { ?>
                        <?php if ( ! in_array( $post_format, array( 'link', 'audio' ) ) ) { ?>
                            <?php if ('on' == $fullwidth):
                                echo '<div class="post_content"><!-- Gui -->';
                            endif; ?>
                            ...
                            if ( 'on' === $args['show_content'] ) {
                                ...
                                if ( has_excerpt() ) {
                                    ...
                                    if ( '' !== $post_content ) {
                                        ...
                                        echo wpautop( truncate_post( 120, false ) );
    ...
    function shortcode_callback( $atts, $content = null, $function_name ) {
        ...
        if ( '' !== $masonry_tile_background_color ) {
            ET_Builder_Element::set_style( $function_name, array(
                'selector'    => '%%order_class%%.et_pb_blog_grid dcm_post .et_pb_post',
                'declaration' => sprintf(
        ...
        if ( have_posts() ) {
            while ( have_posts() ) {
                ...
                $width = 'on' === $fullwidth ? 350 : 400;
                $width = (int) apply_filters( 'et_pb_blog_image_width', $width );

                $height = 'on' === $fullwidth ? 210 : 250;
                $height = (int) apply_filters( 'et_pb_blog_image_height', $height );
                ...
                <article id="post-<?php the_ID(); ?>" <?php post_class( 'et_pb_post dcm_post' . $no_thumb_class . $overlay_class  ); ?>>
                    <?php
                    et_divi_post_format_content();
                        ...
                        if ( ! has_shortcode( $post_content, 'et_pb_blog' ) && ! has_shortcode( $post_content, 'et_pb_portfolio' ) && ! has_shortcode( $post_content, 'et_pb_post_slider' ) && ! has_shortcode( $post_content, 'et_pb_fullwidth_post_slider' ) ) {
                            if ( 'on' === $show_content ) {
                                ...
                                if ( has_excerpt() ) {
                                    ...
                                    echo wpautop( truncate_post( 150, false ) );
                                }
                            }
                        } else if ( has_excerpt() ) {
                            the_excerpt();
